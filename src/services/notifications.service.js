...
export async function fetchNotificationTask(client, taskId) {
  const result = await client.from('tasks').select('id,project_id,title,status,priority,start_date,due_date,created_at,updated_at,deleted_at,assignee_id').eq('id', taskId).maybeSingle();
