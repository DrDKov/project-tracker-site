...
export function ChatPage() {
  ...
  async function sendChatMessage() {
    const textNode = document.getElementById('chatText');
    const fileNode = document.getElementById('chatFiles');
    const text = textNode?.value || '';
    const files = Array.from(fileNode?.files || []);
    await actions.sendChatMessage?.(activeProjectId, text, files);
  ...
  async function clearChat() {
    await actions.clearChatProject?.(activeProjectId);
  }

  return <section className="panel react-chat-page"><ChatPageView model={model} onProjectChange={setActiveProjectId} onSearchChange={(value) => ui.setFilter('chatSearch', value)} onClear={clearChat} onSend={sendChatMessage} onFileCountChange={setFileCount} onDeleteMessage={(id) => actions.deleteChatMessage?.(id)} /></section>;
}
