...
function checkImports(file) {
  ...
  for (const spec of parseLocalImports(js)) {
    ...
    const candidates = [target, `${target}.js`, `${target}.jsx`, `${target}.ts`, `${target}.tsx`, path.join(target, 'index.js'), path.join(target, 'index.jsx'), path.join(target, 'index.ts'), path.join(target, 'index.tsx')];
...
function checkPwa(html) {
  ...
  if (!sw.includes('pt-pwa-') || !sw.includes('CACHE_VERSION')) fail('service-worker.js must define a versioned pt-pwa cache');
  ...
  const pwaClient = path.join(SRC, 'shared', 'pwa', 'pwaClient.ts');
  const pwaLifecycle = path.join(SRC, 'react', 'pwa', 'PwaLifecycle.tsx');
  const pwaRegister = path.join(SRC, 'features', 'pwa', 'register.js');
  for (const file of [pwaClient, pwaLifecycle, pwaRegister]) {
    if (!exists(file)) fail(`Stage 29 PWA file is missing: ${path.relative(ROOT, file)}`);
  }
  const pwaClientTs = fs.readFileSync(pwaClient, 'utf8');
  const pwaLifecycleTsx = fs.readFileSync(pwaLifecycle, 'utf8');
  const registerJs = fs.readFileSync(pwaRegister, 'utf8');
  if (!pwaClientTs.includes("navigator.serviceWorker.register(SERVICE_WORKER_URL, { scope: SERVICE_WORKER_SCOPE })")) fail('PWA client must register ./service-worker.js with ./ scope');
  if (!pwaClientTs.includes('beforeinstallprompt')) fail('PWA client must manage install prompt');
  if (!pwaLifecycleTsx.includes('requestServiceWorkerUpdate')) fail('PWA lifecycle must expose update flow');
  if (!registerJs.includes('addEventListener(\'load\'') && !registerJs.includes('addEventListener("load"')) fail('legacy PWA registration wrapper must still wait for window load');
}
...
if (!html.includes('/src/app/main.tsx')) fail('index.html must load /src/app/main.tsx as the React/Vite entrypoint after Stage 26E');
if (!html.includes('id="root"')) fail('index.html must expose #root after Stage 26E');
if (html.includes('/src/app/bootstrap.js')) fail('index.html must not load legacy bootstrap.js after Stage 26E');
checkPwa(html);
...
if (!pkg.devDependencies?.['@vitejs/plugin-react']) fail('package.json must include @vitejs/plugin-react as devDependency');
if (!pkg.dependencies?.react || !pkg.dependencies?.['react-dom']) fail('package.json must include react and react-dom as dependencies');

console.log(`Project check passed: Vite entrypoint valid, ${jsFiles.length} JS files, ${jsxFiles.length} JSX files, ${tsFiles.length} TS files, ${tsxFiles.length} TSX files, ${cssFiles.length} CSS files, PWA assets valid.`);
