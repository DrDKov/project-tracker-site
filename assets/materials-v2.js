...
(function(){
  ...
  function prof(){try{return window.currentProfile||null}catch(e){return null}}
  ...
  function safe(n){var raw=String(n||'file'),dot=raw.lastIndexOf('.'),ext=dot>0&&/^\.[A-Za-z0-9]{1,12}$/.test(raw.slice(dot))?raw.slice(dot):'',base=ext?raw.slice(0,dot):raw,out='';Array.from(base.normalize('NFKD')).forEach(function(ch){if(/[\u0300-\u036f]/.test(ch))return;if(/[A-Za-z0-9_.',!*&$@=;:+?() -]/.test(ch))out+=ch;else out+='u'+ch.codePointAt(0).toString(16)+'_'});out=out.replace(/\s+/g,'_').replace(/_+/g,'_').replace(/^[._ -]+|[. -]+$/g,'')||'file';return out.slice(0,Math.max(1,120-ext.length))+ext}
  ...
  async function detectOwner(force){
    ...
    var p=prof();
    if(p&&p.id){S.me=p;S.owner=p.role==='owner';return S.owner}
    try{
      var r=await sb.rpc('current_app_user_id');
      if(!r.error&&r.data){
        var u=await sb.from('app_users').select('id,display_name,email,role,is_active').eq('id',r.data).maybeSingle();
        if(!u.error&&u.data){S.me=u.data;S.owner=u.data.role==='owner'&&u.data.is_active!==false;return S.owner}
      }
    }catch(e){}
    try{
      var au=await sb.auth.getUser(), email=au&&au.data&&au.data.user&&au.data.user.email;
      if(email){
        var by=await sb.from('app_users').select('id,display_name,email,role,is_active').ilike('email',email).maybeSingle();
        if(!by.error&&by.data){S.me=by.data;S.owner=by.data.role==='owner'&&by.data.is_active!==false;return S.owner}
      }
    }catch(e){}
  ...
  async function loadAll(){
    ...
    var t=await S.sb.from('workspace_templates').select('*').is('deleted_at',null).order('updated_at',{ascending:false});if(t.error)throw Error(t.error.message);
    var f=await S.sb.from('material_folders').select('*').is('deleted_at',null).order('name',{ascending:true});if(f.error)throw Error(f.error.message);
    var fl=await S.sb.from('material_files').select('*').is('deleted_at',null).order('created_at',{ascending:false});if(fl.error)throw Error(fl.error.message);
    S.templates=t.data||[];S.folders=f.data||[];S.files=fl.data||[];if(!S.folder&&S.folders[0])S.folder=S.folders[0].id;S.loading=false;render();
  }
  ...
  async function saveTemplate(id){var title=($('templateTitle')&&$('templateTitle').value||'').trim(),body=$('templateBody')&&$('templateBody').value||'';if(!title)return alert('Введите название');var r=id?await S.sb.from('workspace_templates').update({title:title,body:body}).eq('id',id).select().single():await S.sb.from('workspace_templates').insert({workspace_id:WID,title:title,body:body,created_by:S.me&&S.me.id}).select().single();if(r.error)throw Error('Не удалось сохранить шаблон: '+r.error.message);S.edit=false;S.tpl=r.data.id;await loadAll()}
  async function delTpl(id){if(!confirm('Удалить шаблон?'))return;var r=await S.sb.from('workspace_templates').update({deleted_at:new Date().toISOString()}).eq('id',id);if(r.error)throw Error(r.error.message);S.tpl=null;await loadAll()}
  ...
  async function newFolder(){var n=prompt('Название папки');if(!n)return;var r=await S.sb.from('material_folders').insert({workspace_id:WID,name:n.trim(),created_by:S.me&&S.me.id}).select().single();if(r.error)throw Error(r.error.message);S.folder=r.data.id;await loadAll()}
  async function renFolder(id){var f=S.folders.find(function(x){return x.id===id});if(!f)return;var n=prompt('Новое название',f.name);if(!n)return;var r=await S.sb.from('material_folders').update({name:n.trim()}).eq('id',id);if(r.error)throw Error(r.error.message);await loadAll()}
  async function delFolder(id){if(!id||!confirm('Удалить папку?'))return;var r=await S.sb.from('material_folders').update({deleted_at:new Date().toISOString()}).eq('id',id);if(r.error)throw Error(r.error.message);S.folder=null;await loadAll()}
  async function upload(file){if(!S.folder)return alert('Сначала выберите папку');var id=crypto.randomUUID(),path=WID+'/'+S.folder+'/'+id+'_'+safe(file.name);var up=await S.sb.storage.from(BUCKET).upload(path,file,{cacheControl:'3600',upsert:false,contentType:file.type||undefined});if(up.error)throw Error('Не удалось загрузить файл: '+up.error.message);var r=await S.sb.from('material_files').insert({id:id,workspace_id:WID,folder_id:S.folder,original_name:file.name,storage_path:path,mime_type:file.type||null,size_bytes:file.size,created_by:S.me&&S.me.id}).select().single();if(r.error){await S.sb.storage.from(BUCKET).remove([path]);throw Error(r.error.message)}await loadAll()}
  async function openFile(id){var f=S.files.find(function(x){return x.id===id});if(!f)return;var r=await S.sb.storage.from(BUCKET).createSignedUrl(f.storage_path,600);if(r.error)throw Error(r.error.message);window.open(r.data.signedUrl,'_blank','noopener')}
  async function delFile(id){var f=S.files.find(function(x){return x.id===id});if(!f||!confirm('Удалить файл?'))return;await S.sb.storage.from(BUCKET).remove([f.storage_path]);var r=await S.sb.from('material_files').update({deleted_at:new Date().toISOString()}).eq('id',id);if(r.error)throw Error(r.error.message);await loadAll()}
